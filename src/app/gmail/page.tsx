...
export default function GmailPage() {
  ...
  const parsedEmails = useMemo(() =>
    emails.map((email: any) => {
      ...
      return {
        ...
        date: dateStr,
        snippet: email.snippet ? decodeHtml(email.snippet) : "No preview available",
      };
    }),
    [emails]
  );
  ...
  const selectedEmail = useMemo(
    () => parsedEmails.find((e: any) => e.id === selectedEmailId) || parsedEmails[0] || null,
    ...
    <div className="flex-1 antialiased flex flex-col h-full animate-in fade-in duration-300" style={{ color: "var(--text-primary)", background: "var(--bg-base)" }}>
      ...
      <header className="px-8 py-6 flex justify-between items-center shrink-0" style={{ borderBottom: "1px solid var(--border)" }}>
        <div>
          ...
          <h1 className="text-2xl font-extrabold tracking-tight mt-1" style={{ color: "var(--text-primary)" }}>Sent Emails</h1>
          <p className="text-xs mt-1" style={{ color: "var(--text-secondary)" }}>View and audit your sent emails.</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => refetchEmails()} disabled={isFetching}
        ...
        <div className="w-1/2 flex flex-col h-full overflow-y-auto" style={{ borderRight: "1px solid var(--border)" }}>
          {isFetching && parsedEmails.length === 0 ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-xs font-bold" style={{ color: "var(--text-secondary)" }}>No Sent Emails Found</p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center gap-2">
                      ...
                      <span className="text-[9px] font-mono shrink-0" style={{ color: "var(--text-muted)" }}>{email.date}</span>
                    </div>
                    <h3 className="text-xs font-bold truncate" style={{ color: isSelected ? "var(--lime)" : "var(--text-primary)" }}>{email.subject}</h3>
                    <p className="text-[11px] line-clamp-2 leading-relaxed" style={{ color: "var(--text-secondary)" }}>{email.snippet}</p>
                  </button>
                );
              })}
            </div>
          ...
          {selectedEmail ? (
            <div className="flex flex-col h-full justify-between">
              <div className="space-y-6">
                <div className="space-y-3 pb-5" style={{ borderBottom: "1px solid var(--border)" }}>
                  <div className="flex justify-between items-start gap-4">
                    <h2 className="text-sm font-bold leading-snug" style={{ color: "var(--text-primary)" }}>{selectedEmail.subject}</h2>
                    <span className="text-[9px] font-mono shrink-0" style={{ color: "var(--text-secondary)" }}>{selectedEmail.date}</span>
                  </div>
                  <div className="space-y-1">
                    {[{ label: "From", val: selectedEmail.from }, { label: "To", val: selectedEmail.to }].map(({ label, val }) => (
              ...
              <div className="pt-6 flex items-center justify-between gap-4" style={{ borderTop: "1px solid var(--border)" }}>
                <span className="text-[9px] font-mono" style={{ color: "var(--text-muted)" }}>ID: {selectedEmail.id}</span>
              </div>
            </div>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
              ...
              <p className="text-xs font-medium" style={{ color: "var(--text-muted)" }}>Select an email to preview details.</p>
            </div>
          )}
          {fetchError && (
